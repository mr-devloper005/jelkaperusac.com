      ...
      <main>
        ...
        <section className="px-3 pt-4 sm:px-5 sm:pt-6">
          <div className="relative mx-auto w-full max-w-[var(--editable-container)] overflow-hidden rounded-[2rem] bg-[var(--slot4-ink)] px-6 py-12 text-[var(--slot4-on-ink)] sm:rounded-[2.5rem] sm:px-10 sm:py-16 lg:px-14">
            <div className="pointer-events-none absolute -right-28 -top-28 h-80 w-80 rounded-full bg-[var(--slot4-accent)]/25 blur-[120px]" />
            <div className="relative grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--slot4-accent)]">{pagesContent.search.hero.badge}</p>
                <h1 className="editable-display mt-4 text-3xl font-bold leading-[1.08] tracking-[-0.02em] sm:text-4xl lg:text-5xl">{pagesContent.search.hero.title}</h1>
              ...
              <form action="/search" className="rounded-[1.5rem] bg-white/5 p-4 backdrop-blur-sm sm:p-5">
                ...
                <button className="mt-3 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-bold text-white transition hover:brightness-95" type="submit">
